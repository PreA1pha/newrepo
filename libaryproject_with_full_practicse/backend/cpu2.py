...
class Process():
    ...
    def read(self):
        self.sql.execute("SELECT * FROM Lib")
        self.rd=self.a.fetchall()
        for tup in self.rd:
            for word in tup:
                print(word)#neden görmedi anlamadım
    def updt(self):
        ...
        self.sql.execute("UPDATE Lib SET '{} '= '{}' WHERE Id='{}'".format(self.namenew,self.pagenew,self.old))
        return self.sql.commit()
    def cl(self):
        self.sql.close()

vook1 = Process("walter","300","white","10/10")
